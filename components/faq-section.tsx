...
/**
 * FAQ entries are built lazily so that internal links can be prefixed with the active locale.
 * Each function takes a `prefix` (e.g. `/pl`, `/en`) which is composed with the existing slug.
 */
const buildFaqPL = (prefix: string) => [
  {
    question: 'Czym dokładnie jest "AI Safety"?',
    answer:
      "AI Safety to szeroka dziedzina badawcza skupiająca się na redukcji wielu rodzajów ryzyk i zagrożeń związanych z zaawansowaną sztuczną inteligencją. Jednym z najsłynniejszych problemów jest tzw. 'alignment', czyli możliwość sterowania systemów AI zgodnie z intencjami ich twórców. AI Safety obejmuje również badania nad kwestiami takimi jak: wpływ systemów AI na ekonomię i społeczeństwo, zapobieganie wykorzystaniu AI do niepożądanych celów (np. cyberataki), regulacje prawne, a nawet świadomość AI.",
  },
  {
  ...
  },
  {
    ...
    answer:
      "Zapraszamy do kontaktu bezpośrednio przez nasz e-mail: contact@aisafety.org.pl Chętnie udzielamy wywiadów i komentarzy na temat bezpieczeństwa AI.",
  },
]

const buildFaqEN = (prefix: string) => [
  {
  ...
  },
  {
    ...
    answer:
      "Please contact us directly via our email: contact@aisafety.org.pl. We are happy to give interviews and comments on AI safety.",
  },
]
...
export function FaqSection() {
  ...
  const prefix = `/${lang}`
  const items = lang === "pl" ? buildFaqPL(prefix) : buildFaqEN(prefix)
